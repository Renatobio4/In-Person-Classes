...
class db():
    def rag(query_embedding):
        ...
        WHERE embedding MATCH ?
            AND k = 5
        ORDER BY distance;
    """, [serialize(query_embedding)]).fetchall()
...
def index():
    ...
    if query:
        ...
        rag = db.rag(query_embedding)
        rag = sorted(rag, key=lambda item: item[3], reverse=True)
    ...
    return template('index', query=query, response=response, rag=rag)
